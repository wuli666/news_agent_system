...
async def main_supervisor_node(state: State):
    ...
    if last_agent == "collector":
        ...
        next_node = "collection_team_coordinator"
    ...
    # Collection team finished, check if need analysis
        ...
        if iteration >= max_iterations:
            ...
            decision = "summarize"
            next_node = "summary_team_coordinator"
        ...
        # Check if we have enough basic collection
            ...
            next_node = "analysis_team_coordinator"
    ...
    # Analysis team finished, check quality
        ...
        if not gap_pending and (quality_ok or iteration_limit or has_research or has_sentiment or has_relationship):
            decision = "summarize"
            next_node = "summary_team_coordinator"
...
"""
    ...
    try:
        ...
        if information_gaps:
            ...
            for gap in information_gaps[:3]:  # 最多Extract 3条
                ...
                for news_item in text_news:
                    if news_item.get("title", "") == gap_title:
                        target_news.append(news_item)
                        break
...
识别主要趋势和未来走向。"""
    ...
    try:
        ...
        return Command(update={"trend_analysis": None}, goto="summary_chart_agent")
...
请根据以上信息生成一份结构化的新闻报告。
在输出字段中务必填充 timeline_analysis、trend_analysis、relationship_graph，直接复用或凝练上面的对应分析，避免留空。"""
    ...
    try:
        ...
        report_json = report_obj.model_dump_json(indent=2, ensure_ascii=False)
        ...
        fallback_report = f"报告生成失败: {str(exc)}"
        try:
